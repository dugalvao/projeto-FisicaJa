...
<html lang="pt-br">
	...
	<div class="container-fluid col-10 col-sm-10 col-md-10 col-xl-10 p-3 mt-3">
		...
		<div class="container col-12 col-sm-12 col-md-10 col-xl-10" id="divs">
			<h3><?php echo "c = $valor1 / $valor2 . ( $valor3 - $valor4 )";?></h3><br/>
			<?php $util1 = $valor3 - $valor4 ;?>
			<h3><?php echo "c = $valor1 / $valor2 . $util1 "?></h3><br/>
			<?php $util2 = $valor2 * $util1 ;?>
			<h3><?php echo "c = $valor1 / $util2 "?></h3><br/>
			<?php
				if($util2 == 0){
					$result = 0;
				}else{
				...
				if($result < 0){
					...
					$resp = $_SESSION['RespostaNegativaCalor'];
				}else{?>
					<h3><?php
					$resultForm = number_format($result, 2,  ',', '');
					echo "c = " .$resultForm. "cal/g.°C "; 
					$resp = "";
					?></h3>
				<?php } ?>
	...
	<script>
			function salvar(){	
				...
				$.ajax({
					url: "salvar13.php",//pagina em php que ira fazer o insert dos dados
